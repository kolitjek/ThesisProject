...
class BaseGame(Entity):
	...
	def __init__(self, players, scenario = None):
		self.data = None
...
class CoinRules:
	...
	def pick_first_player(self, scenario):
		if scenario == None:
			...
			if scenario.starting_player == "player1":
				self.log("Scenario... player1 starts")
				return self.players[0], self.players[1]
			elif scenario.starting_player == "player2":
				self.log("Scenario... player2 starts")
				return self.players[1], self.players[0]
			else:
				print("Scenario startingPlayer has an invalid name. Printed in Game.py l. 373")
				return None
